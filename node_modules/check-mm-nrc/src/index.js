...
const MyanmarCitizenType = Object.freeze({
    'Citizen': 'C',
    'National': 'C',
    'Associate': 'A',
    'Naturalised': 'Nat',
    getLong(short = '') {
        ...
        const data = Object.entries(this).find(([key, value]) => value);
        return data[0] || null;
    }
})
...
function checkMyanmarNRC(nrc = '') {
    ...
    const nrcReg = new RegExp(/[0-9]+\/[A-Za-z]+\([A-Za-z]\)[0-9]+/i);
    const parenthes = new RegExp(/\([A-Za-z]\)/i);
    const nullOb = {
        type: null,
        number: null,
        township: null,
    ...
    if (nrc.length == 0 || !nrc.match(nrcReg)) {
        return nullOb;
    }

    const slashIndex = nrc.indexOf('/');

    const state = Number(nrc.substring(0, slashIndex));
    const citizenshipType = nrc.match(parenthes)[0].substring(1, 2).toUpperCase();
    const township = nrc.substring(slashIndex + 1, nrc.indexOf('('));
    const number = nrc.substring(nrc.lastIndexOf(')') + 1, nrc.length);


    if (state < 0 && state > 12) return nullOb;


    const townshipData = MyanmarNRCData.filter(nrcdata => {
        return nrcdata.name_en.toUpperCase() == township.toUpperCase() && Number(nrcdata.nrc_code) == state
    })

    return {
        type: citizenshipType,
        typeInLetter: MyanmarCitizenType.getLong(citizenshipType),
        number,
        township,
        state,
